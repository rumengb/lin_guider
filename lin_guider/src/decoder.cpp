 ...
 * decoder.cpp
 *
 * Based on original code from guvcview http://guvcview.sourceforge.net/
 *
 *
...
{\
    ...
    r = CROP((y + r_add) >> SCALEBITS);\
    g = CROP((y + g_add) >> SCALEBITS);\
    b = CROP((y + b_add) >> SCALEBITS);\
}
...
/*
 * Note: Work only for image multiple of 2
 */
    						...
    						unsigned char *d)
{
    ...
    for(;height >= 2; height -= 2) {
        ...
        for(w = width; w >= 2; w -= 2) {
            YUV_TO_RGB1_CCIR(pu[0], pv[0]);
            // output 4 pixels
            YUV_TO_RGB2_CCIR(r, g, b, py[0]);
            RGB_OUT(d1, r, g, b);

            YUV_TO_RGB2_CCIR(r, g, b, py[1]);
            RGB_OUT(d1 + 4, r, g, b);
...
static void convert_yuv420p_to_422(int * out,unsigned char *pic,int width);
static void convert_yuv422p_to_422(int * out,unsigned char *pic,int width);
static void convert_yuv444p_to_422(int * out,unsigned char *pic,int width);
static void convert_yuv400p_to_422(int * out,unsigned char *pic,int width);
...
static int isInitHuffman = 0;
static struct jpeg_decdata *decdata = NULL;
...
int jpeg_decode(unsigned char **pic, unsigned char *buf, int *width, int *height)
{
    ...
    if (i != 0 || j != 63 || m != 0) {
    	log_e("hmm FW error,not seq DCT ??\n");
    }
   // printf("ext huffman table %d \n",isInitHuffman);
    ...
    if( intwidth != *width || intheight != *height || *pic == NULL )
    {
    	...
    	*height = intheight;
    	...
    	size_t len = intwidth * (intheight + 8) * 2;
    	*pic = (unsigned char *) realloc((unsigned char *)*pic,	len);
    	memset(*pic, 0, len);
    }
	...
	ypitch = 16 * pitch;
	convert = convert_yuv420p_to_422;
	break;
    case 0x21: //422
   // printf("find 422 %dx%d\n",*width,*height);
	...
	pitch = *width * bpp; // YUYV out
	ypitch = 8 * pitch;
	convert = convert_yuv422p_to_422;
	break;
    case 0x11: //444
	mcusx = *width >> 3;
	mcusy = *height >> 3;
	...
	ypitch = 8 * pitch;
	 if (info.ns==1) {
    		mb = 1;
		convert = convert_yuv400p_to_422;
	} else {
		mb=3;
		convert = convert_yuv444p_to_422;
	}
        break;
    default:
	err = ERR_NOT_YCBCR_221111;
...
unsigned int convert_yuv422_to_rgb32(unsigned char * input_ptr, unsigned char * output_ptr, unsigned int image_width, unsigned int image_height)
{
	...
	for (i = size; i > 0; i--) {
		...
		*output_pt++ = R_FROMYV(Y,V);
		output_pt++;	// remove this line for rgb24
