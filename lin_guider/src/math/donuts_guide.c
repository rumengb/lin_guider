...
int dgh_new_frame_digest(const double *fdata, const int width, const int height, frame_digest *fdigest) {
	int i, ci, li, max;
	double (*col_x)[2];
	double (*col_y)[2];
	double total, avg;
	...
	fdigest->fft_x = malloc(2 * fdigest->width * sizeof(double));
	if (fdigest->fft_x == NULL) return -1;

	fdigest->fft_y = malloc(2 * fdigest->height * sizeof(double));
	if (fdigest->fft_y == NULL) {
		free(fdigest->fft_x);
		return -1;
	}
	...
	/* Flatten the X peak */
	avg = total / fdigest->width * 1.05;
	for(i=0; i < fdigest->width; i++) {
		col_x[i][RE] = (col_x[i][RE] >= avg) ? avg : col_x[i][RE];
	}

	/* Flatter the Y peak */
	avg = total / fdigest->height * 1.05;
	for(i=0; i < fdigest->height; i++) {
		col_y[i][RE] = (col_y[i][RE] >= avg) ? avg : col_y[i][RE];
	}
